...
(function(){
  ...
  function renderCheckout(){
    ...
    // Panels toggle by radio
    // Keep only Revolut method in UI (safety even if HTML cached)
    document.querySelectorAll('.pay-option').forEach(lbl=>{
      const inp = lbl.querySelector('input[name="pay"]');
      if(!inp) return;
      if(inp.value !== 'revolut'){
        const panel = document.getElementById(`panel-${inp.value}`);
        if(panel) panel.remove();
        lbl.remove();
      }
    });
    // Ensure Revolut panel is visible
    const revPanel = document.getElementById('panel-revolut');
    if(revPanel) revPanel.style.display = '';
    ...
    // Real checkout via Stripe endpoint
    // Revolut-only checkout
  ...
  // Revolut-only enforcement on any page load that has payment UI (defensive)
  (function enforceRevolutOnly(){
    const opts = document.querySelectorAll('.pay-option input[name="pay"]');
    if(!opts.length) return; // not on checkout
    opts.forEach(inp=>{
      if(inp.value !== 'revolut'){
        document.getElementById(`panel-${inp.value}`)?.remove();
        inp.closest('.pay-option')?.remove();
      }else{
        inp.checked = true;
        const revPanel = document.getElementById('panel-revolut');
        if(revPanel) revPanel.style.display = '';

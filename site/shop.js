...
(function(){
  ...
  function renderCheckout(){
    ...
    function togglePanels(){
      ...
      document.querySelectorAll('.pay-pill').forEach(lbl=> lbl.classList.remove('active'));
      const checked = document.querySelector(`.pay-pill input[value="${sel}"]`);
      if(checked && checked.closest) checked.closest('.pay-pill').classList.add('active');
    }
    ...
    const inBuyerEmail = document.getElementById('inBuyerEmail');
    try{
      ...
      if(inBuyerEmail && saved.buyerEmail){ inBuyerEmail.value = saved.buyerEmail; window.BUYER_EMAIL = saved.buyerEmail; }
    }catch{}
    function persist(){
      const cfg = {
        ...
        buyerEmail: (inBuyerEmail && inBuyerEmail.value || ''),
      };
    ...
    [inRev, inPme, inDirect, inBuyerEmail].forEach(el=>{
      if(!el) return;
    ...
    btn.addEventListener('click', ()=>{
      ...
      if(t <= 0){ if(err){ err.textContent = 'Il carrello è vuoto.'; err.style.display = ''; } if(note){ note.style.display='none'; } return; }
      // Compose optional email with download/guide links
      function sendEmail(){
        const email = (window.BUYER_EMAIL||'').trim();
        if(!email) return;
        const items = loadCart();
        const lines = items.map(it => {
          const link = (window.DOWNLOAD_LINKS||{})[it.id] || '';
          return `- ${it.name} (${currency(it.price)}) ${link?"→ "+link:""}`;
        }).join('%0D%0A');
        const subject = encodeURIComponent('Il tuo ordine CAT WARE');
        const body = `Ciao,%0D%0A%0D%0AGrazie per il tuo acquisto. Ecco i dettagli:%0D%0A${lines}%0D%0A%0D%0ASe hai bisogno di aiuto, rispondi a questa email.`;
        const cc = (window.MERCHANT_EMAIL||'').trim();
        const ccParam = cc ? `&cc=${encodeURIComponent(cc)}` : '';
        const href = `mailto:${encodeURIComponent(email)}?subject=${subject}&body=${body}${ccParam}`;
        // Open mail client in a new tab to allow redirect to continue
        window.open(href, '_blank');
      }
      if(method === 'paypalme'){
        const user = (window.PAYPAL_ME||'').trim().replace(/^@+/, '');
      ...
      if(direct){
        ...
        const user = (window.REVOLUT_USER || '').trim().replace(/^@+/, '');
